...
// numRows * (cellSize + 1) + 1, 0, 0, img.getWidth(), img.getHeight(), null);
    ...
    private void drawBackground(Graphics2D g2) {
        if (grid instanceof GridMap) {
            @SuppressWarnings("unchecked")
            Grid<Actor> grid = ((GridMap) this.grid).getLayer(MapLayer.FloorLevel);
    ...
    private void drawGridlines(Graphics2D g2) {
        ...
        int maxy = Math.min(numRows, (curClip.y + curClip.height - top + cellSize) / (cellSize + 1)) * (cellSize + 1) + top;
        int maxx = Math.min(numCols, (curClip.x + curClip.width - left + cellSize) / (cellSize + 1)) * (cellSize + 1) + left;
    ...
    private void recalculateCellSize(int minSize) {
        if (numRows == 0 || numCols == 0) {
            ...
            int desiredCellSize = Math.min((viewableSize.height - extraHeight()) / numRows, (viewableSize.width - extraWidth()) / numCols) - 1;
