...
public class PlayerActor extends Actor implements CompleatStats, PathedImage {
    ...
    public static final File file = new File("resource//image//entities//PlayerActor.gif");
...
// i've added a return statement to prevent the game crashing on death but the
// game will still crash if there are two entities attacking the player
// if the player presses C the player actor will skip their turn
// Pressing the WASD keys will still move but no longer through walls and if
// there is an enemy in the way the player will attack it
...
// config
        ...
        if (YSPL.keys.keys[KeyEvent.VK_D] == true) {
            ...
            if (tar != null) {
                ...
                if (super.getGrid().isMoveable(loc)) {
                    moveTo(loc);
                }
            }
            return true;
